...
from model.losses import EWCLoss
...
def _train(args):
    ...
    if args.load_model:
        ...
        fisher[args.task] = dict()
        params[args.task] = dict()
    else:
        fisher, params = dict(), dict()
        fisher[args.task], params[args.task] = dict(), dict()
    ...
    if len([x for x in fisher.keys() if x != args.task]) > 0:
        ewc_citerion = EWCLoss(fisher=fisher, params=params, ewc_lambda=LAMBDA)
        weights.append(1)
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        for _ in range(args.steps_per_epoch):
            ...
            if len([x for x in fisher.keys() if x != args.task]) > 0:
                ewc_loss = ewc_citerion.loss(adapter.named_parameters())
                if not freeze_all:
                    ewc_loss += ewc_citerion.loss(model.named_parameters())
                loss = seg_loss + adapt_loss_bbox + adapt_loss_samples + ewc_loss
        ...
        wandb_log["Train EWC Loss"] = np.mean(epoch_ewc_loss)
    ...
    for m in [model,adapter] if not freeze_all else [adapter]:  # Only when not frozen
        for name, param in m.named_parameters():
            # -- Update the fisher and params dict -- #
            if param.grad is None:
                fisher[args.task][name] = torch.tensor([1], device='cuda:0')
            else:
                fisher[args.task][name] = param.grad.data.clone().pow(2)
