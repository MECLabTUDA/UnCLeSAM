...
def _train(args):
    ...
    # train_lists = [list(v) for _, v in itertools.groupby(train_files, key=lambda x:x.split(os.sep)[6])]  # --> task names and IDs
    ...
    if args.load_model:
        ...
        model = Own_Sam.load(args.load_model, device)
    else:
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        for _ in range(args.steps_per_epoch):
            ...
            y_pred_, samples_coords_for_loss, bbox_coords_for_loss, _, _ = model(imgs, embeds, orig_sizes, input_sizes, bboxs_gt, samples_gt, train=True)
        ...
        _, _, _, val_res, _ = validate_no_adapter(model, val_files, epoch, store_samples=False, out_=os.path.join(args.model_dir, "validation"))

        wandb_log["Train Loss"] = np.mean(epoch_loss)
        wandb_log["Train Dice"] = np.mean(dice)
        wandb_log["Val Dice"] = val_res["Dice"]
        wandb_log["Val IoU"] = val_res["IoU"]
    ...
    # validate_no_adapter(model, val_files, args.epochs, store_samples=False, out_=os.path.join(args.model_dir, "validation"))
    print("")
