...
task = 'Task99_HarP' #  Task97_DecathHip Task98_Dryad  Task99_HarP
model_type = 'vit_b'
...
parser.add_argument('-i', '--nii_path', type=str, default=f'/local/scratch/maflal/{task}/imagesTr', help='path to the nii images')
parser.add_argument('-gt', '--gt_path', type=str, default=f'/local/scratch/maflal/{task}/labelsTr', help='path to the ground truth',)
parser.add_argument('-o', '--npz_path', type=str, default=f'/local/scratch/maflal/{task}/{model_type}/Tr', help='path to save the npz files')
...
def preprocess_nonct(gt_path, nii_path, gt_name, image_name, label_id, image_size, sam_model, device):
    ...
    image_data_pre = (image_data_pre - np.min(image_data_pre))/(np.max(image_data_pre)-np.min(image_data_pre))*255.0 # Normalizing
